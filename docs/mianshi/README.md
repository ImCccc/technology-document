...
参考文档：<https://juejin.cn/post/6914198504822800391>；性能界面：<https://googlechrome.github.io/devtools-samples/jank/>

**查出问题**

使用 chrome 控制台 Performance 界面生成性能报告，里面分析了浏览器每一个时间段都做了什么，调用函数的时长，白屏时间等等；一般通过下面几点分析：
...
3. 减少代码体积： js css 代码压缩，路由懒加载，删除项目中无用的代码
4. 利用浏览器缓存：对于不常改变的静态资源使用 CDN，浏览器有缓存再次加载会很快；同时减少主文件打包体积
5. 减少请求数量： HTTP 请求从建立到关闭需要花费时间，将一些信息合并到一个接口返回，可以作为优化点
6. 图片优化： 采用 svg 图片或者字体图标，图片懒加载
7. 样式优化：尽可能避免重排和重绘，少使用 offsetTop table 布局等等
8. 下拉刷新（针对移动端）
9. 上万条数据显示，使用虚拟列表
10. 滚动事件，输入事件使用节流防抖
11. 服务器配置 gzip 压缩
...
1. 了解项目需求，使用人群，项目周期；浏览器兼容性（是否支持 IE）；对于移动端，是否需要访问手机原生功能，是否支持 iphone；项目是面向 C 端还是 B 端，这些都需要去衡量；例如如果是支持 IE，那么就不能使用 vue3 或者最新的 react；如果经常需要访问手机原生功能，那么就需要考虑是否使用 RN，或者 flutter
2. 了解公司目前项目使用的技术栈；例如公司统一使用 vue，那保持一致性是很有必要的，这样可以资源最大化利用，公司积累下来的公共组件或者函数也可以复用；
3. 做技术调研，主要是调研：
   > 1. 各个框架周边生态是否完善，完善的生态，方便实现很多常见的功能
   > 2. 使用的人群数量，社区是否活跃，使用的人越多，社区越活跃，遇到问题越容易解决
   > 3. 框架是否一直都在更新维护；尽可能不要使用停止维护的框架，一旦遇到框架本身的问题就很难解决
4. 了解组员的基本情况：了解同事熟悉的技术，针对性技术选型，可以减少项目延期的风险
