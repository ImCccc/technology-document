...
1. 减少代码体积： js css 代码压缩，路由懒加载，删除项目中无用的代码
2. 利用浏览器缓存：对于不常改变的静态资源使用 CDN，浏览器有缓存再次加载会很快；同时减少主文件打包体积
3. 减少请求数量： HTTP 请求从建立到关闭需要花费时间，将一些信息合并到一个接口返回，可以作为优化点
4. 图片优化： 采用 svg 图片或者字体图标，图片懒加载
5. 下拉刷新（针对移动端）
6. 上万条数据显示，使用虚拟列表
7. 滚动事件，输入事件使用节流防抖
8. 服务器配置 gzip 压缩
